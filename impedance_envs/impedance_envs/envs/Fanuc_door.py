...
class Fanuc_pivoting(gym.Env):
    def __init__(self, render=True):
        ...
        self.sim = ctypes.cdll.LoadLibrary(cwd + "/impedance_envs/impedance_envs/envs/bin/mujocosim_door.so")
        ...
        self.nv = 8 + 2  # subject to change according to different tasks
        ...
        self.work_space_origin = 0*np.array([0.65, 0.0, 0.0])
        ...
        self.action_vel_low = -0.1 * np.array([1, 1, 1])
    ...
    def reset(self):
        # set eef init pose
        init_c_pose = np.array([0.3, -0.1, 0.35, 0.5*np.pi, 0*np.pi, 0*np.pi])
    ...
    def step(self, action):
        ...
        # desired_kp = 200 * np.array([1, 1, 1])
        # desired_vel = np.array([0,0,-1])
    ...
    def set_joint_velocity(self, target_vel, threshold=0.2):
        T = 1 / self.HZ
        vcl_norm = np.linalg.norm(target_vel, ord=2)
        if vcl_norm > threshold:
            target_vel = target_vel / vcl_norm * threshold
    ...
    def admittance_control(self, ctl_ori=False, clip_vel=False):
        ...
        MA = 0*world_force + np.multiply(self.adm_kp, e) + np.multiply(self.adm_kd, e_dot)
