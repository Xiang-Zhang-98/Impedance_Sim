...
void init(void)
{
    ...
    char error[1000];
    m = mj_loadXML("./impedance_envs/impedance_envs/envs/source/LRMate_200iD_pivoting.xml", 0, error, 1000); // MuJoCo model
    ...
    if (Render){
        ...
        window = glfwCreateWindow(1800, 1200, "Fanuc Robot Pivoting Simulation", NULL, NULL);
...
//        mju_copy(nv, mjtNum(m->nv), 1);
    ...
    void get_obj_pose(mjtNum *obj_pose)
    {
        mjtNum obj_pos[3];
        mjtNum obj_rot_mat[9];
        // mjtNum obj_rot_quat[4];
        // mjtNum jacp[24];
        // mjtNum jacr[24];
        // mjtNum Full_jacobian[48];
        // mjtNum obj_vel[6];
        int geom_id = mj_name2id(m, mjOBJ_GEOM, "object_geom_1");
        
        for (int j = 0; j < 3; j++)
            obj_pos[j] = d->geom_xpos[3 * geom_id + j];
        for (int j = 0; j < 9; j++)
            obj_rot_mat[j] = d->geom_xmat[9 * geom_id + j];
        for (int j = 0; j < 3; j++)
            obj_pose[j] = obj_pos[j];
        for (int j = 0; j < 9; j++)
            obj_pose[j + 3] = obj_rot_mat[j];
    }
}