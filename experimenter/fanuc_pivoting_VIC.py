...
class insertion_primitive(object):
    ...
    def experiment(self, init_time):
        ...
        filename = 'exp_data/'+init_time+'/'+current_time+'square.pkl'
        if self.record:
...
if __name__ == "__main__":
    ...
    Path("exp_data/"+current_time+"/Fance_pivoting_vic").mkdir(parents=True, exist_ok=True)
    filename = 'exp_data/'+current_time+"/Fance_pivoting_vic"+'/init_time.pkl'
    with open(filename, 'wb') as f:
        joblib.dump(current_time, f)
    for i in range(10):
        prm.reset()
        prm.experiment(current_time+"/Fance_pivoting_vic")
